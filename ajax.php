...
if($action == 'save_scores') {
    ...
    try {
        ...
        // Validate inputs
        if(empty($student_id) || empty($quiz_id) || !is_numeric($score)) {
            echo json_encode(['status' => 0, 'msg' => 'Invalid input data']);
            exit;
        }
        
        // Check if record exists
        $check = $conn->query("SELECT id FROM quiz_student_list WHERE student_id = '$student_id' AND quiz_id = '$quiz_id'");
        
        if($check->num_rows > 0) {
            // Update existing record
            $update = $conn->query("UPDATE quiz_student_list SET score = '$score', total_score = '$total_score', date_updated = NOW() WHERE student_id = '$student_id' AND quiz_id = '$quiz_id'");
            
            if($update) {
                echo json_encode(['status' => 1, 'msg' => 'Score updated successfully']);
            } else {
                error_log("Update failed: " . $conn->error);
                echo json_encode(['status' => 0, 'msg' => 'Failed to update score: ' . $conn->error]);
            }
        } else {
            // Insert new record
            $insert = $conn->query("INSERT INTO quiz_student_list (student_id, quiz_id, score, total_score, date_updated) VALUES ('$student_id', '$quiz_id', '$score', '$total_score', NOW())");
            
            if($insert) {
                echo json_encode(['status' => 1, 'msg' => 'Score saved successfully']);
            } else {
                error_log("Insert failed: " . $conn->error);
                echo json_encode(['status' => 0, 'msg' => 'Failed to save score: ' . $conn->error]);
            }
        }
        
    } catch(Exception $e) {
        error_log("Exception in save_scores: " . $e->getMessage());
        echo json_encode(['status' => 0, 'msg' => 'Server error: ' . $e->getMessage()]);
    }
    exit;
}